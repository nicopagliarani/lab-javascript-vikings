...
// Saxon
class Saxon extends Soldier{receiveDamage(damage){this.health-=damage;
...
vikingAttack(){let vikingRandom = Math.floor(Math.random() * this.vikingArmy.length);
...
saxonAttack(){let vikingRandom = Math.floor(Math.random() * this.vikingArmy.length);
  let randomViking = this.vikingArmy[vikingRandom];
...
showStatus() {if(this.saxonArmy<=0){return 'Vikings have won the war of the century!';}
